...
const booksContainer = document.querySelector('.search_results');

function renderBooks() {
    articles.forEach(article => {
        const bookTab = document.createElement('div');
        bookTab.classList.add('books_tabs');

        const bookInfo = `
            <section class="book_info">
                <h2 class="release_date">${article.date}</h2>
                <h3 class="age_range">${article.ages}</h3>
                <h3 class="book_genre">${article.genre}</h3>
                <h3 class="book_rating">${article.stars}</h3>
            </section>
        `;
        ...
        const bookSummary = `
            <section class="book_summary">
                <h1 class="book_name">${article.title}</h1>
                <img class="book_img" src="${article.imgSrc}" alt="${article.imgAlt}">
                <p class="book_summary">${article.description}</p>
            </section>
        `;


        bookTab.innerHTML = bookInfo + bookSummary;

        booksContainer.appendChild(bookTab);
    });
}
